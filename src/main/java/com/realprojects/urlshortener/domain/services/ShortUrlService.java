...
import com.realprojects.urlshortener.domain.repositories.ShortUrlRepository;
...
public class ShortUrlService {
    ...
    public ShortUrlDto createShortUrl(CreateShortUrlCmd cmd) {
        if(properties.validateOriginalUrl()) {
            // a utility service created to hit a url and check it's existence based on response code of http request
        ...
        if(cmd.userId() == null) {// conditionally set to default values if guest user is creating short url
    ...
    public Optional<ShortUrlDto> accessShortUrl(String shortKey, Long userId) {
        ...
        // Checking if given short url exists or not
        if(shortUrlOptional.isEmpty()) {
            return Optional.empty();
        }
        ShortUrl shortUrl = shortUrlOptional.get();
        //Verifying if given short url hasn't expired
